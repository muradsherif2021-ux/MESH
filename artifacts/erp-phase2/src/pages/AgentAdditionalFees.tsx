...
interface AgentAdditionalFee {
  id: string; number: string; agentId: string; agentNameAr?: string; agentCode?: string;
  date: string; feeCategory?: string; description: string; operationRef?: string;
  totalAmount: string; paymentMethod: string; dueDate?: string; status: string; notes?: string;
}
interface FormData {
  agentId: string; date: string; feeCategory?: string; description: string;
  totalAmount: string; paymentMethod: string; dueDate?: string;
  operationRef?: string; branchId?: string; notes?: string;
}

export default function AgentAdditionalFees() {
  const { hasPermission } = useAuth();
  const [data, setData] = useState<{ data: AgentAdditionalFee[]; pagination: any } | null>(null);
  ...
  const [editing, setEditing] = useState<AgentAdditionalFee | null>(null);
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await api("/agent-additional-fees").get({ page, limit: 20, search: search || undefined, status: statusFilter || undefined } as any);
      setData(res as any);
  ...
  const openEdit = (r: AgentAdditionalFee) => { setEditing(r); reset({ ...r } as any); setError(""); setModalOpen(true); };

  const onSubmit = async (values: FormData) => {
    setSaving(true); setError("");
    try {
      if (editing) await api(`/agent-additional-fees/${editing.id}`).put(values);
      else await api("/agent-additional-fees").post(values);
      setModalOpen(false); load();
    } catch (err: any) { setError(err.message); } finally { setSaving(false); }
  };

  const handlePost = async (id: string) => {
    if (!confirm("هل تريد تأكيد وترحيل هذا السجل؟")) return;
    setPosting(id);
    try {
      await api(`/agent-additional-fees/${id}/post`).post({});
      load();
    } catch (err: any) { alert(err.message); } finally { setPosting(null); }
  };

  const columns = [
    { key: "number", headerAr: "الرقم", render: (r: AgentAdditionalFee) => <span className="font-mono text-xs text-primary font-bold">{r.number}</span> },
    { key: "date", headerAr: "التاريخ", render: (r: AgentAdditionalFee) => <span className="text-xs">{r.date}</span> },
    { key: "agentNameAr", headerAr: "وكيل الشحن", render: (r: AgentAdditionalFee) => <span className="text-sm">{r.agentNameAr ?? "—"}</span> },
    { key: "feeCategory", headerAr: "الفئة", render: (r: AgentAdditionalFee) => <span className="text-xs text-muted-foreground">{r.feeCategory ?? "—"}</span> },
    { key: "description", headerAr: "البيان", render: (r: AgentAdditionalFee) => <span className="text-sm truncate max-w-[160px] block">{r.description}</span> },
    { key: "totalAmount", headerAr: "المبلغ", render: (r: AgentAdditionalFee) => <span className="font-semibold text-sm ltr">{Number(r.totalAmount).toLocaleString("ar-SA", { minimumFractionDigits: 2 })} ر.س</span> },
    { key: "paymentMethod", headerAr: "طريقة الدفع", render: (r: AgentAdditionalFee) => <span className="text-xs">{paymentMethodLabels[r.paymentMethod]}</span> },
    { key: "status", headerAr: "الحالة", render: (r: AgentAdditionalFee) => <StatusBadge status={r.status} /> },
    { key: "actions", headerAr: "", render: (r: AgentAdditionalFee) => (
      <div className="flex items-center gap-1">
        {r.status === "DRAFT" && hasPermission("agent_additional_fees", "edit") && (
          <button onClick={e => { e.stopPropagation(); openEdit(r); }} className="p-1.5 hover:bg-muted rounded text-muted-foreground hover:text-foreground"><Pencil size={13} /></button>
        )}
        {r.status === "DRAFT" && hasPermission("agent_additional_fees", "post") && (
          <button onClick={e => { e.stopPropagation(); handlePost(r.id); }} disabled={posting === r.id}
            className="p-1.5 hover:bg-green-500/10 rounded text-green-600 hover:text-green-700">
  ...
  return (
    <div dir="rtl">
      <PageHeader
        titleAr="الرسوم الإضافية لوكلاء الشحن"
        subtitleAr="تسجيل الرسوم الإضافية لوكلاء الشحن — تدخل في مجموعة التكاليف القابلة للاسترداد"
        action={hasPermission("agent_additional_fees", "create") ? (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <div className="p-5 border-b border-border">
              <h2 className="text-base font-bold">{editing ? "تعديل رسوم إضافية" : "إضافة رسوم إضافية لوكيل شحن"}</h2>
            </div>
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium mb-1">فئة الرسوم</label>
                  <input type="text" {...register("feeCategory")} placeholder="مثال: رسوم تخزين..."
                    className="w-full px-3 py-2 text-sm rounded-lg border border-input bg-background focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" {...register("description", { required: true })} placeholder="وصف الرسوم..."
                    className="w-full px-3 py-2 text-sm rounded-lg border border-input bg-background focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div>
