...
interface Treasury { id: string; code: string; nameAr: string; nameEn?: string; currency: string; currentBalance: string; isActive: boolean; }

export default function Treasuries() {
  const [data, setData] = useState<{ data: Treasury[]; pagination: any } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [res, accs] = await Promise.all([
        api("/treasuries").get({ limit: 50 } as any),
  ...
  const openCreate = () => { setEditing(null); reset({ currency: "SAR", isActive: true }); setError(""); setModalOpen(true); };
  const openEdit = (t: Treasury) => { setEditing(t); reset({ ...t }); setError(""); setModalOpen(true); };

  const onSubmit = async (values: any) => {
    setSaving(true); setError("");
    try {
      if (editing) await api(`/treasuries/${editing.id}`).put(values);
      else await api("/treasuries").post(values);
  ...
  const columns = [
    { key: "code", headerAr: "الكود", render: (r: Treasury) => <span className="font-mono text-xs font-bold text-primary">{r.code}</span> },
    { key: "nameAr", headerAr: "الاسم" },
    { key: "currency", headerAr: "العملة", render: (r: Treasury) => <span className="text-xs font-mono font-bold">{r.currency}</span> },
    { key: "currentBalance", headerAr: "الرصيد الحالي", render: (r: Treasury) => (
    ...
    { key: "isActive", headerAr: "الحالة", render: (r: Treasury) => <StatusBadge status={r.isActive ? "ACTIVE" : "INACTIVE"} /> },
    { key: "actions", headerAr: "", render: (r: Treasury) => (
  ...
  return (
    <div dir="rtl">
      <PageHeader titleAr="الخزائن" subtitleAr="إدارة الخزائن النقدية وأرصدتها"
        action={<button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90"><Plus size={15} /> إضافة خزينة</button>}
      />
      <DataTable columns={columns} data={data?.data ?? []} pagination={data?.pagination} isLoading={isLoading} emptyMessageAr="لا توجد خزائن بعد" />
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-2xl border border-border shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <h3 className="font-bold">{editing ? "تعديل خزينة" : "إضافة خزينة جديدة"}</h3>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <F label="العملة"><select {...register("currency")} className={inputCls}><option value="SAR">ريال سعودي (SAR)</option><option value="USD">دولار أمريكي (USD)</option><option value="EUR">يورو (EUR)</option></select></F>
              </div>
